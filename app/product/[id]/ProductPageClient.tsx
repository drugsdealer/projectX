      ...
      <main className="flex-grow pt-6 md:pt-20 pb-32 md:pb-20 lg:pb-24 px-3 sm:px-6 lg:px-8 mx-auto w-full max-w-[1800px] relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 items-start w-full max-w-none gap-10 lg:gap-12">
          ...
          >
            ...
            {/* ── Universal brand block: single brand OR collab ── */}
            {(() => {
              ...
              if (!primaryBrand && !isCollab) return null;

              // All brands combined: main brand first, then collab
              const allBrands = [
                ...(primaryBrand ? [{ id: (product as any)?.brandId ?? 0, name: primaryBrand, slug: (product as any)?.brandSlug ?? brandSlugFrom(primaryBrand), logoUrl: brandLogoSrc ?? null }] : []),
                ...collabBrands,
              ];

              const label = isCollab ? 'Бренды' : 'Бренд';
              const displayText = isCollab
                ? allBrands.map((b) => b.name).join(' × ')
                : `Больше от ${primaryBrand}`;

              if (!isCollab) {
                // Single brand → plain link
                return (
                  <div className="mt-4">
                    <Link
                      href={brandHref}
              ...
              // Collab → button with dropdown
              return (
                <div className="mt-4 relative" ref={collabRef}>
                  <button
                    type="button"
                    onClick={() => setCollabOpen((v) => !v)}
                    className="group flex w-full items-center gap-5 rounded-xl border border-black/10 bg-white px-6 py-6 md:py-7 min-h-[120px] hover:border-black/25 hover:bg-black/[0.02] transition-all duration-200 text-left"
                  >
                    {/* Overlapping logos */}
                      ...
                      {allBrands.map((b, i) => (
                        ...
                        >
                          {b.logoUrl ? (
                            ...
                            <span className="text-sm font-bold text-black/60">{b.name.charAt(0)}</span>
                          )}
              ...
              <div className="my-6">
                ...
                {relatedColorProducts.length > 5 && (
                  <button
                    ...
                    className="mt-3 text-sm text-gray-500 hover:text-black underline underline-offset-4 transition-colors"
                  >
                    Ещё расцветок ({relatedColorProducts.length - 5})
                  </button>
                )}
              </div>
            )}
                ...
                {colorSheetOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      drag="y"
                      dragControls={colorSheetDragControls}
                      dragListener={false}
