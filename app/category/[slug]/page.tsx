...
function prettyCategoryTitle(raw: string) {
  ...
  const dict: Record<string, string> = {
    ...
    shoulderbag: 'Сумка через плечо',
    shoulderbags: 'Сумки через плечо',
    crossbody: 'Кроссбоди',
    crossbodybag: 'Кроссбоди',
    crossbodybags: 'Кроссбоди',
    hobo: 'Хобо',
    hobobag: 'Хобо',
    hobobags: 'Хобо',
    bucket: 'Бакет',
    bucketbag: 'Бакет',
    bucketbags: 'Бакет',
    handbag: 'Сумка в руки',
    handbags: 'Сумки в руки',
    minibag: 'Мини-сумка',
    minibags: 'Мини-сумки',
...
export default async function CategoryPage({
  ...
  const SUBCATEGORY_SYNONYMS: Record<string, string[]> = {
    ...
    totes: ['tote', 'totes'],
    shoulderbags: ['shoulderbag', 'shoulderbags'],
    crossbodybags: ['crossbody', 'crossbodybag', 'crossbodybags'],
    hobobags: ['hobo', 'hobobag', 'hobobags'],
    bucketbags: ['bucket', 'bucketbag', 'bucketbags'],
    handbags: ['handbag', 'handbags'],
    minibags: ['minibag', 'minibags'],
  ...
  const PARENT_CATEGORIES: Record<string, string[]> = {
    ...
    bags:         ['bags', 'backpacks', 'waistbags', 'cardholders', 'wallets', 'totes', 'travelbags', 'shoulderbags', 'crossbodybags', 'hobobags', 'bucketbags', 'handbags', 'minibags'],
  ...
  const INFER_RULES: Array<{ slug: string; rx: RegExp }> = [
    ...
    { slug: 'shoulderbags', rx: /(сумк.*через\s*плеч|плечев.*сумк|shoulder\s*bag)/i },
    { slug: 'crossbodybags', rx: /(кроссбоди|cross[\s-]?body)/i },
    { slug: 'hobobags',    rx: /(хобо|hobo)/i },
    { slug: 'bucketbags',  rx: /(бакет|bucket\s*bag)/i },
    { slug: 'minibags',    rx: /(мини[\s-]?сумк|mini[\s-]?bag)/i },
    { slug: 'handbags',    rx: /(хендбег|handbag)/i },
